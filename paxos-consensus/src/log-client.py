...
def put_log_entries(sc):
    ...
    print("Finished sending log entries, exiting in 15 seconds...")
    time.sleep(15)
    ...
    time.sleep(10)
    cmp_r = utils.cmp_log(output_id)

    results.append({
        'scenario': scenario,
        'sim_crashes': sc.simultaneous_crashes,
        'entries': sc.total_entries,
        'suc_entr': len(sc.local_entries),
        'warnings': warnings,
        'result': cmp_r,
    })
    
    df = pd.DataFrame(results)
    df.to_csv(f'results/scenario{scenario}_results.csv', mode='a', index=False, header=not os.path.exists(f'results/scenario{scenario}_results.csv'))
    time.sleep(3)
    os._exit(0)
